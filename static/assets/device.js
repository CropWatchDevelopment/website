/* CropWatch — CW-AIR-TH 3D viewer (Babylon.js).
   Loads the real binary STL of the sensor case, centers + frames it,
   applies a white-plastic material, auto-rotates, and lets the user drag.
   Falls back to a procedural enclosure if the STL can't load.
   (Ported from the claude.ai/design "Japan Website" project; STL rootUrl
   adjusted to the absolute /assets/ path for SvelteKit.) */
...
(function () {
  ...
  // ── Camera ──
  const camera = new BABYLON.ArcRotateCamera('cam', -Math.PI / 2.3, 1.15, 14, BABYLON.Vector3.Zero(), scene);
  ...
  function addContactShadow(yBottom, span) {
    const disc = BABYLON.MeshBuilder.CreateDisc('shadow', { radius: span * 0.62, tessellation: 48 }, scene);
    ...
    sm.diffuseTexture = dt; sm.diffuseTexture.hasAlpha = true;
  ...
  function done() { if (loaderEl) loaderEl.style.display = 'none'; }
  ...
  function buildFallback() {
    ...
    lid.position.y = 1.06; lid.material = mat; lid.parent = root;
    const probe = BABYLON.MeshBuilder.CreateCylinder('probe', { height: 1.0, diameter: 0.5 }, scene);
    probe.rotation.z = Math.PI / 2; probe.position.set(-2.6, -0.2, 0); probe.material = mat; probe.parent = root;
    ...
    lm.emissiveColor = new BABYLON.Color3(0.05, 0.6, 0.38); lm.disableLighting = true;
    led.material = lm; led.parent = root;
  ...
  BABYLON.SceneLoader.ImportMeshAsync('', '/assets/', 'sensor_case_body.stl', scene)
    .then((res) => {
      ...
      meshes.forEach((m) => {
        m.parent = root;
        m.material = mat;
        try { m.createNormals(true); } catch (e) {}
      });
