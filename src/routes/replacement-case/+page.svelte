<script lang="ts">
	...
	// 価格を公開したら productSchema({ path, price, offerUrl }) で復活。
...
<Breadcrumbs
	items={[{ label: 'ホーム', href: '/' }, { label: '製品' }, { label: '交換用ケース' }]}
/>
		...
		<div class="hero__media" data-reveal>
			<div class="stage">
				...
				<span class="stage__hint"
					><span class="material-symbols-rounded">drag_pan</span> ドラッグで回転 ・ スクロールで拡大</span
				>
			</div>
		</div>
		<div class="hero__copy" data-reveal>
			<p class="eyebrow">
				<span class="material-symbols-rounded">deployed_code</span> 交換用ケース（CW-CASE）
			</p>
			<h1>過酷な現場から、<br /><span class="hl">中身を守る頑丈なケース。</span></h1>
			<p class="hero__kicker">
				冷凍庫の霜も、鶏舎の洗浄も、畑の直射日光も。<span class="u"
					>中の電子部品をしっかり守ります。</span
				>
			</p>
			<p class="hero__lead">
				CropWatch のすべてのセンサーが収まる<b>防塵・防水（IP66）ケース</b
				>。割れたり傷んだりしても、<b>校正済みのモジュールはそのまま新しいケースへ</b
				>。マウスのドラッグで、3Dでぐるりと確認できます。
			</p>
			...
			<div class="hero__ctas">
				...
				>
				<a href="/replacement-sensors" class="btn btn--ghost btn--lg"
					><span class="material-symbols-rounded">cable</span> センサーを見る</a
				>
			</div>
			<p class="hero__micro">
				<span class="material-symbols-rounded">verified</span> 取り付け方法やアンテナの選び方もご提案します。
			</p>
		</div>
	</div>
</section>
	...
	<div class="wrap">
		<div class="section__head" data-reveal>
			<p class="eyebrow eyebrow--accent">
				<span class="material-symbols-rounded">shield</span> どんな場所でも
			</p>
			<h2>1つのケースで、あらゆる環境に。</h2>
		</div>
...
<section class="section">
	<div class="wrap split">
		<div data-reveal>
			<p class="eyebrow">
				<span class="material-symbols-rounded">extension</span> 入れ替えできる設計
			</p>
			<h2>ケースを替えても、校正はそのまま。</h2>
			<p class="split__lead">
				ケースが割れたり、傷んだりしても、校正済みのセンサーを失うことはありません。新しいケースにモジュールを移すだけで、そのまま使い続けられます。
			</p>
			<ul class="checklist">
				<li>
					<span class="material-symbols-rounded">check</span> 校正済みモジュールをそのまま移せる
				</li>
				<li><span class="material-symbols-rounded">check</span> 工具なしで開け閉めできる</li>
				<li>
					<span class="material-symbols-rounded">check</span> 取り付け・アンテナの選択肢も豊富
				</li>
			</ul>
		</div>
...
<section class="section section--soft scroll-pad" id="spec">
	<div class="wrap">
		<div class="section__head" data-reveal>
			<p class="eyebrow"><span class="material-symbols-rounded">description</span> 仕様</p>
			<h2>CW-CASE のスペック</h2>
		</div>
...
<style>
	.stage {
		position: relative;
		background: radial-gradient(circle at 50% 30%, #f0f5ff, var(--web-bg-soft));
	...
	.stage__badge .material-symbols-rounded {
		font-size: 17px;
	}
	.stage__hint {
		position: absolute;
	...
	.stage__hint .material-symbols-rounded {
		font-size: 17px;
	}
	.stage__loader {
		position: absolute;
