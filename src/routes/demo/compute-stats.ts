/** Verbatim port of CropWatch/src/lib/utils/computeStats.ts. */
...
/**
 * Stats for a newest-first series (the order the API — and demo-history —
 * returns rows in). `computeStats` takes `lastReading` from the END of the
 * array, which on a newest-first series is the OLDEST sample, so the caller has
 * to say which end is current. AirDisplay does exactly this in the app; the
 * demo applies it to the soil stat cards too, where the app currently does not.
 */
export function computeStatsNewestFirst(values: number[]): CwStatCardData {
	if (values.length === 0) return {};
	return { ...computeStats(values), lastReading: values[0], trend: 'up' };
}
