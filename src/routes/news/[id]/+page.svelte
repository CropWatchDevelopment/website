<script lang="ts">
	...
	import { DEFAULT_OG_IMAGE, absUrl } from '$lib/seo/site';
	...
	const seoTitle = $derived(`${article.title} | News | CropWatch`);
	const ogImage = $derived(article.headerImage ?? DEFAULT_OG_IMAGE);
...
<article class="news-article">
	<div class="wrap news-article__in">
		<header class="news-head" data-reveal>
			<p class="news-cat">Announcement</p>
			<h1>{article.title}</h1>
			<p class="news-meta">
		...
		<p class="news-back"><a href="/news">← Back to all news</a></p>
	</div>
</article>
