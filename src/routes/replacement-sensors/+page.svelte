...
<section class="section section--tint" id="sensorTypes">
	<div class="wrap">
		<div class="section__head" data-reveal>
			<p class="eyebrow eyebrow--accent"><span class="material-symbols-rounded">sensors</span> 4つのモジュール、ひとつの仕組み</p>
			<h2>設定不要でかんたん交換</h2>
		...
		<div class="picker" data-reveal>
			...
			<div class="picker-panel">
				...
				<dl class="picker-specs">
					{#each active.specs as [k, v], i (i)}<div class="picker-spec"><dt>{k}</dt><dd>{v}</dd></div>{/each}
				</dl>
			</div>
		</div>
	</div>
...
<style>
	...
	.picker-tab { display: flex; align-items: center; gap: 13px; text-align: left; cursor: pointer; font-family: inherit;
		...
		box-shadow: var(--web-shadow-card); transition: border-color 0.18s, transform 0.18s, background 0.18s; }
	...
	.picker-panel { display: flex; flex-direction: column; gap: 14px; background: var(--web-surface);
		...
		padding: 24px 26px; }
	.picker-panel__tag { align-self: flex-start; font-size: 11px; font-weight: 700; letter-spacing: .05em; font-family: var(--cw-font-mono);
		color: var(--web-primary); background: var(--web-primary-soft); padding: 5px 11px; border-radius: 9999px; }
	.picker-panel h3 { font-size: 1.25rem; margin: 0; line-height: 1.25; }
