<script lang="ts">
	...
	// scene timings below are written in warped time and stay untouched.
	const HOLD_AT = [2.6, 5.2, 8.0];
	const warp = (rr: number) => {
		let w = rr;
		for (const h of HOLD_AT) if (w > h) w = Math.max(h, w - DEMO_HOLD);
		return w;
	};
	...
	let cap = $derived(
		r > 1.7 && r < 3.2
			? 'A sensor stops responding → error'
			: r > 4.3 && r < 5.8
				? 'A sensor returns an error → error'
				: r > 7.0 && r < 8.8
					? 'The two readings disagree → error'
					: r > 10.6 && r < 16
						? 'When both agree, the validated value is published'
						: ''
	);
	...
	let g3vals = $derived.by(() => {
		...
		return {
			...
			delta: disagree ? 'Δ 2.7 °C  ✕  exceeds ±0.3 °C' : 'Δ 0.1 °C  ✓  within ±0.3 °C',
			ok: !disagree
		};
	});
...
{#if t >= F0 - 0.2 && t <= 29.2 + FLOW_SHIFT}
	<div style="position:absolute; inset:0; opacity:{sceneFade};">
		<!-- Keep this under ~22 chars: the eyebrow box shrink-wraps against the
		     540px of canvas right of centre and wraps to two lines past that. -->
		<Eyebrow text="The validation process" x={540} y={56} color={C.teal} />
		...
		<Gate
			cx={SPINE}
			cy={g1}
			label="Did both sensors respond?"
			tone={g1tone}
		...
		/>
		<ErrorPill cx={errX} cy={g1} opacity={o1} reason="Sensor offline" />
		<Line
			x={SPINE - 2}
			y={g1 + 31}
			len={g2 - 31 - (g1 + 31)}
		...
		<Gate
			cx={SPINE}
			cy={g2}
			label="Did either report an error?"
			sub="Both must return a value"
			tone={g2tone}
		...
		/>
		<ErrorPill cx={errX} cy={g2} opacity={o2} reason="Measurement error" />
		<Line
			x={SPINE - 2}
			y={g2 + 31}
			len={g3 - 55 - (g2 + 31)}
		...
		<Gate
			...
			w={400}
			h={110}
			label="Cross-check: do the values agree?"
			tone={g3tone}
		...
		/>
		<ErrorPill cx={errX} cy={g3} opacity={o3} reason="Validation error" />
		<Line
			x={SPINE - 2}
			y={g3 + 55}
		...
		{#if rin > 0}
			...
			>
				...
				<div>
					...
					>
						{lit ? 'Validated reading' : 'Publishes the primary value'}
					</div>
					<div
